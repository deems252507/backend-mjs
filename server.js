...
app.get('/api/init', async (req, res) => {
    try {
        ...
        await pool.query(`CREATE TABLE IF NOT EXISTS retur_records (
         ...
         // === TAMBAHAN: Alter table jika tabel lama tidak punya kolom exchange_items ===
        try { 
            await pool.query('ALTER TABLE retur_records ADD COLUMN exchange_items JSON'); 
        } catch(e) { /* Kolom sudah ada, abaikan error */ }
...
// === PUT /api/transaction/edit-struk ===
// DIPERBAIKI: Tidak lagi bergantung pada ID dari frontend. Menggunakan ID asli dari DB berdasarkan urutan index.
app.put('/api/transaction/edit-struk', async (req, res) => {
    ...
    try {
        ...
        // Ambil ID asli dari database diurutkan ASC
        const [trxRows] = await conn.query(
            'SELECT id, nomor_transaksi FROM transactions WHERE nomor_transaksi = ? ORDER BY id ASC',
            [invoice]
        );
        if (trxRows.length === 0) {
            await conn.rollback();
        ...
        // Update harga berdasarkan urutan index (lebih aman dari masalah presisi ID)
        for (let i = 0; i < trxRows.length; i++) {
            if (i < items.length) {
                const dbId = trxRows[i].id; // ID asli dari database
                const newHarga = items[i].harga_satuan;
                
                await conn.query(
                    'UPDATE transactions SET harga_satuan = ? WHERE id = ?',
                    [newHarga, dbId]
                );

                // Update tax_records terkait
                ...
                for (const tax of taxRows) {
                    const newSubtotal = newHarga * tax.jumlah;
                    ...
                    await conn.query(
                        ...
                        [newHarga, newSubtotal, newNilaiPajak, tax.tax_id]
                    );
                }
            }
        }
