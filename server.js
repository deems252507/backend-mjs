...
// Fungsi untuk memastikan kolom exchange_items ada
async function ensureReturColumns() {
    try {
        ...
        console.error("Gagal cek/tambah kolom exchange_items:", e.message); 
    }
}
...
// DIPERBAIKI: Menggunakan ID asli dari database berdasarkan urutan item (index)
app.put('/api/transaction/edit-struk', async (req, res) => {
    ...
    try {
        ...
        // Jika jumlah items cocok, gunakan ID asli dari DB (berdasarkan index)
        // Ini menghindari masalah presisi ID dari frontend
        if (trxRows.length === items.length) {
            ...
            // Fallback jika jumlah tidak cocok (misal ada item yang dihapus)
            for (const item of items) {
                ...
                if (itemId > 0) {
                    ...
                    const [taxRows] = await conn.query('SELECT tax_id, jumlah, persentase_pajak FROM tax_records WHERE trx_id = ?', [itemId]);
                    for (const tax of taxRows) {
...
app.post('/api/transaction/retur', async (req, res) => {
    ...
    try {
        // Pastikan kolom exchange_items ada
        await ensureReturColumns();
...
app.put('/api/settings', async (req, res) => {
    ...
    try {
        ...
        if (returRecords && Array.isArray(returRecords) && returRecords.length > 0) {
            await ensureReturColumns(); // Pastikan kolom ada sebelum insert
            const values = returRecords.map(r => [
                r.id, r.parent_invoice, new Date(r.tanggal), r.kasir||'', r.pelanggan||'',
                JSON.stringify(r.items || []), JSON.stringify(r.exchange_items || [])
            ]);
            await pool.query(
