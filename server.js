...
app.get('/api/login-bootstrap', async (req,res)=>{
    try {
        ...
        await pool.query(`CREATE TABLE IF NOT EXISTS app_settings (
            ...
            master_pajak JSON NULL,
            users JSON NULL,
            shift_sessions JSON NULL,
        ...
        for (const [column, definition] of [
            ...
            try {
                ...
                console.warn('[LOGIN BOOTSTRAP] migrasi kolom users:', e.message);
            }
        }
        ...
        let [users] = await pool.query(`
            SELECT username,password,role,name,shift,status,aktif,data
            FROM users
            ORDER BY username
        `);

        // Migrasi akun lama dari app_settings hanya jika tabel users kosong.
        if (!users.length) {
            ...
            if (Array.isArray(legacy) && legacy.length) {
                for (const u of legacy) {
                    const username = safeString(u.username || u.id).trim();
                    if (!username || !safeString(u.password).trim()) continue;

                    try {
                        await pool.query(`
                            INSERT IGNORE INTO users
                            (username,password,role,name,shift,status,aktif,data)
                            VALUES (?,?,?,?,?,?,?,?)
                        `, [
                            username,
                        ...
                        console.warn(
                            '[LOGIN BOOTSTRAP] migrasi user lama:',
                            e.message
                        );
                    }
                }
        ...
        // Jika database benar-benar belum memiliki akun, gunakan akun bawaan
        // hanya di memory. Database tidak ditimpa.
        if (!users.length) {
        ...
        try {
            const [rows] = await pool.query(`
                SELECT *
                FROM shift_sessions
                ORDER BY COALESCE(start_time,'1000-01-01') DESC,id DESC
            `);
            shifts = rows;
        } catch (e) {
            console.warn(
                '[LOGIN BOOTSTRAP] shift_sessions:',
                e.message
            );
        }
        ...
        res.json({
            ...
            users:users.map(u => ({
                ...
                shift:u.shift || '',
                status:u.status ||
                    (Number(u.aktif) === 1 ? 'Aktif' : 'Nonaktif'),
                aktif:Number(u.aktif) === 1
            })),
            shiftSessions:shifts.map(r => ({
                ...safeJSON(r.data,{}),
                id:r.id,
                username:r.username,
                name:r.name,
        ...
        console.error('[LOGIN BOOTSTRAP]',error);
        res.status(503).json({
            success:false,
            error:error.message
        });
